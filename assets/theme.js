...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _updateAddToCart: function(evt) {
      ...
      variant_crosschecker(this.variants.product,variant.options);
      ...
      if (variant) {
        ...
        if (variant.available) {
          if( $('option[data-vid="'+variant.id+'"]').is(':disabled') ) {
...
function variant_crosschecker(product_json,variant_options){
  var product_options = {},
      product_option_1 = [],
      product_option_2 = [],
      product_option_3 = [],
      product_variants = product_json.variants,
      prod_id = product_json.id;
  
  $('.swatch[data-option-id="'+ prod_id +'"] .swatch-element').addClass('soldout');
  
  $.each(product_variants, function(index, value){
    product_option_1.push(value.option1);
    product_option_2.push(value.option2);
    product_option_3.push(value.option3);
  });
  
  var option_1_values = product_option_1.filter(function(value, i, option_1_values) {
        return i == option_1_values.indexOf(value);
      }),
      option_2_values = product_option_2.filter(function(value, i, option_2_values) {
        return i == option_2_values.indexOf(value);
      }),
      option_3_values = product_option_3.filter(function(value, i, option_3_values) {
        return i == option_3_values.indexOf(value);
      });
  
  product_options['option1'] = option_1_values;
  product_options['option2'] = option_2_values;
  product_options['option3'] = option_3_values;
  
  $.each(product_options,function(i,curr_options_array){
    if(curr_options_array.length == 1 && curr_options_array[0] == null) {
      return true;
    }
    var variant_options_downcase = $.map(variant_options, function(n,i){return n.toLowerCase();}),
        curr_options_array_downcase = $.map(curr_options_array, function(n,i){return n.toLowerCase();});
    var intersects = $.map(variant_options_downcase,function(a){return $.inArray(a, curr_options_array_downcase) < 0 ? null : a;}),
        initial_check = variant_options_downcase.slice(0);
    if (intersects.length != 0) {
      initial_check = initial_check.filter(function(x) { return intersects.indexOf(x) < 0 });
    }
    $.each(curr_options_array_downcase,function(index,option_value){
      var clone = initial_check.slice(0);
      clone.push(option_value);
      var array_compare = $.grep(product_variants, function (element, index) {
        return arrayContainsArray($.map(element.options, function(n,i){return n.toLowerCase();}),clone);
      });
      console.log('option_value - '+ option_value);
      console.log(array_compare[0]);
      if(array_compare.length > 0 && array_compare[0].available){
        $('.swatch[data-option-id="'+ prod_id +'"] .swatch-element[data-value="'+ option_value +'"]').removeClass('soldout');
      }
    });
  });
}
